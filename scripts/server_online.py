...
class LastKServer(Server):
    ...
    def _session_enqueue_metadata(self, body: dict) -> dict:
        """Predict whether this enqueue will start a training round (last-K warmup aware)."""
        del body  # metadata is derived from queue/history counters only
        queued = self.sessions_queue.qsize()
        future_history = len(self._session_history) + queued
        if future_history < self._min_training_sessions:
            return {
                "training_triggered": False,
                "warmup": True,
                "history_len": future_history,
                "min_sessions": self._min_training_sessions,
            }
        future_pending = self.training_queue.qsize() + 1
        triggered = future_pending >= self.config.update_every_n_sessions
        return {
