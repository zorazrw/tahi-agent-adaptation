...
def extract_session(cursor: sqlite3.Cursor, session_id: str, *, db_root: Optional[Path] = None) -> Optional[dict]:
    try:
        row = cursor.execute(
...
def main() -> int:
    ...
    try:
        ...
        if args.session_id:
            payload = extract_session(cursor, args.session_id, db_root=db_root)
            if not payload:
                print(f"Error: session not found: {args.session_id}", file=sys.stderr)
                return 1
        else:
            payload = extract_all_sessions(cursor, db_root=db_root)
