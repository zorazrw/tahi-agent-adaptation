...
Requires Anthropic credentials (same as `induce.py`). Grading uses `claude-haiku-4-5` by default (`--model` to override). Prints per-criterion PASS/FAIL and `average_success_rate` (passes / total criteria).
